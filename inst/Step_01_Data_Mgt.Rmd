...
```{r plot_periods, fig.height=9, fig.width=7.5, fig.cap=FCap}
# Figure caption.
...
```{r update_F1, fig.height=9, fig.width=7.5}

Scenarios %>% 
  filter(type == "comment") %>% 
  mutate(label = paste(event, criteria, sep = ": ")) ->
  SLabels
...
F1 + theme(text = element_text(size = 15, color = "black")) +
  ...
  annotate(geom = "text", x = as_datetime("2014-02-14"), y = 1, size = 3, 
              label = "Some days unobserved", hjust = 0) 
