...
```{r create_ARREWAgg3}
# This solves warnings issued by max() when it is used on an empty vector. 
# max(numeric(0)) yields warning "no non-missing arguments to min; returning -Inf"
# So we replace the max() function with one that just returns NA when the vector
# is empty. This speeds up execution dramatically
...
IDNEW %>% 
  ...
  # Merge in ACat12 incident count variables broken down by IWhen 
  left_join(x = ., y = ARREWAgg3, by = "OID") %>% 
  # Merge in CCat12 incident count variables broken down by IWhen 
  left_join(x = ., y = CHGEWAgg3, by = "OID") %>% 
  # Merge in JCat12 incident count variables (for convictions only) broken down by IWhen 
