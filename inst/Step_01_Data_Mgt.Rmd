...
even the year of collection is known, but most that of the cases with missing 
SDate values have the year of collection recorded in *SDate.Yr* and some also
have the month recorded in *SDate.Mt*.
...
partial date information, then SDate is missing but SDate.Yr and possibly also
SDate.Mt have valid values, but we cannot assign a precise date to the SAK
collection and *WDate* ends up with a missing value.
...
Here we pause to assess how many of the offenders with CHR data have complete 
...
```{r examine_WDate}
# Table caption.
T1cap <- paste("Number of SAKs by WDate Status for Earliest SAK Associated",
               "with Perpetrator")
# Footnote text.
FN1 <- paste("Cells contain counts of perpetrators. Only perpetrators for whom",
...
addmargins(xtabs(~OID_NSAK + WDate.Status, addNA = TRUE, data = SPDCHRE)) %>% 
  ...
  kable(., format = "latex", booktabs = TRUE, caption = T1cap, 
...
perpetrators with CHR data. Rather than try to impute WDate for the remaining 
`r nrow(SPDCHRE) - nrow(SPDCHREW)` perpetrators, we will exclude them from the 
sample analyzed for this paper. 

```{r}
SPDCHRE %>% 
  #filter(is.na(WDate)) %>% 
  select(SAKID, OID, OID_NSAK, SDate, SDate.Miss, SDate.Mt, OID_IDate, WDate, 
         WDate.Yr, WDate.Mt, WDate.Status) ->
  SPDCHRE2
