...
data, *WDate* can in principle either precede or occur on *Age16Date* . We
create variables below that help us determine how often that occurred and adjust
the before period date interval in a sensible way when it does.
...
```{r scenarios_table}
# Table caption.
TCap <- paste("Scenarios for Alignment of Before, During, and After Testing", 
              "Window Periods with Observed Criminal History Records Period.")
# Footnote text.
FN <- paste0("Offsets from specific dates (e.g., - 1) are in units of days.")

Scenarios %>% 
  filter(type == "comment") %>% 
  select(scenario, event, criteria) %>% 
  kable(format = "latex", booktabs = TRUE, 
        col.names = c("Scenario", "Description", "Criteria"),
        caption = TCap) %>% 
  kable_styling() %>% 
  footnote(general = FN, general_title = "Note: ", footnote_as_chunk = TRUE,
         threeparttable = TRUE)
```
Now we save a vector of scenario labels for use later. 

```{r scenario_labels}
Scenarios %>%
  filter(type == "comment") %>% 
  select(scenario, event) %>% 
  mutate(Scenario = paste(scenario, event, sep = ". ")) %>% 
  pull(Scenario) -> 
  ScenarioLabels
```
...
SPDCHREW %>% 
  ...
  mutate(
    ...
    Scenario = case_when(
      Age16Date < WDate - 1 ~ "A. Before (all), During (all), After (all)",
      Age16Date == WDate - 1 ~ "B. Before (1 day), During (all), After (all)",
      Age16Date == WDate ~ "C. Before (none), During (all), After (all)",
      WDate < Age16Date & Age16Date < WDate + 118 ~ "D. Before (none), During (part), After (all)",
      Age16Date == WDate + 118 ~ "E. Before (none), During (1 day), After (all)",
      Age16Date == WDate + 119 ~ "F. Before (none), During (none), After (all)",
      Age16Date > WDate + 118 ~ "G. Before (none), During (none), After (part)"), 
    # Convert to a factor for later convenience. 
    Scenario = factor(Scenario, levels = ScenarioLabels, 
...
= 0 years. Table \ref{tab:freq_scenario}
...
IDNEW %>% 
  ...
  count() %>% 
  mutate(Percent = 100*n/nrow(IDNEW)) %>% 
   kable(format = "latex", booktabs = TRUE, digits = 2, 
        col.names = c("Scenario", "Frequency", "Percent"),
        caption = TCap) 
```
