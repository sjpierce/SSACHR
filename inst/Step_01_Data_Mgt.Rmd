...
Here we aggregate the dummy coded crime category variables data from the record 
...
# Aggregate ACat12 dummy codes to incident level binary variables.
ARREW %>% 
  group_by(OID, IID) %>% 
...
# Aggregate CCat12 dummy codes to incident level binary variables.
CHGEW %>% 
  group_by(OID, IID) %>% 
  # Aggregate dummy codes by OID + IID into binary variables
...
# Aggregate JCat12 dummy codes to incident level binary variables.
CONEW %>% 
  group_by(OID, IID) %>% 
  # Aggregate dummy codes by OID + IID into binary variables
