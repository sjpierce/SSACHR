...
read_sav(here::here("./inst/extdata/SAK_PERP_2018-04-03.sav"), 
         user_na = TRUE) %>% 
  # Sort by OID and SAK collection date. 
  arrange(OID, SDate) ->
  SPD
```
...
each offender and store that as a new dataset. Along the way, we need to do 
...
perpetrators for whom we have criminal history data. 

```{r}
SPD %>% 
  # Retain only only offenders with CHR data. 
  filter(OID %in% IDN$OID) %>% 
  # 
