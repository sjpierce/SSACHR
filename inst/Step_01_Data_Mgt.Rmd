...
paper about the criminal histories of suspected serial sexual offenders. The 
data come from Detroit, MI. 
...
* During a *testing window* that represents a working definition for what 
  constitutes timely forensic testing.
...
```{r create_SPDCHR}
SPD %>% 
  # Retain only only offenders with CHR data. 
  filter(OIDinCHR == "Yes") -> 
  SPDCHR 
```

With the resulting SPDCHR data, we need to identify the start of the testing 
window for each SAK and store it in a new variable called WDate, then aggregate 
across SAKs associated with each offender to identify the earliest WDate value. 

### Identify Start of Testing Window for each SAK
Creating the new WDate variable for recording when the testing window for each
SAK started is complicated by the fact that some SAKs have only partial date 
information, leaving SDate with NA values. 

We also need to account for the fact that in our prior work we linked some SAKs
to existing CHR incident records that involved criminal sexual conduct (as
evidenced by an associated arrest offense record, prosecutor charge record, or
judicial charge record for a sexual assault). Those links were created recognize 
possible overlap between sexual assaults represented in the CODIS hits (CHITS)
data from the SAK testing and the CHR data files. However, we did not require 
exact date matches in the previous work because we were dealing wth data that 
originated from different organizations. SAK collection dates (SDate) were 
recorded by hospitals (or other healthcare providers) who actually collected the 
SAKs. These dates are more likely to be accurate than the incident dates on 
because healthcare providers have a very strong financial incentive to get dates 
of service correct for billing purposes. 
...
the SAK collection date (SDate) and WDate can be set equal to SDate if SDate is 
non-missing. 

```{r}
SPDCHR %>% 
  mutate(WDate = case_when(
  ...
  # Group by OID, then select first record for each offender and save a tibble. 
  group_by(OID) %>% 
  filter(row_number() == 1) %>% 
  ungroup() %>% 
  # Add another variable. 
  mutate(WDate.Miss = case_when(
           is.na(WDate) == TRUE & is.na(WDate.Yr) == TRUE ~ "M",
           is.na(WDate) == TRUE & is.na(WDate.Mt) == TRUE ~ "PDY",
           is.na(WDate) == TRUE & is.na(WDate.Mt) == FALSE ~ "PDYM",
           is.na(WDate) == FALSE ~ "C")) ->
  SPDCHRE
```

```{r examine_WDate_missingness}
addmargins(xtabs(~OID_NSAK + WDate.Miss , addNA = TRUE, data = SPDCHRE))
```
...
SPDCHRE %>% 
  #filter(is.na(WDate)) %>% 
  select(SAKID, OID, OID_NSAK, SDate, SDate.Miss, SDate.Mt, OID_IDate, WDate, 
         WDate.Yr, WDate.Mt, WDate.Miss) ->
  SPDCHRE2
  
as.data.frame(SPDCHRE2)


  
```

Now we capture a vector of OIDs for offenders who have multiple SAKs at least
one of which has a completely missing SDate or a partial SDate. 

```{r}
SPDCHR %>% 
  filter(OID_NSAK > 1 & SDate.Miss != "C") %>% 
  pull(OID) %>% 
  unique() -> 
  OID_MSAKs_SDateMissing
```

Inspect the SAKs and associated SDate values for that subset of offenders. 

```{r inspect_missing_SDate}
SPDCHRE %>% 
  filter(OID %in% OID_MSAKs_SDateMissing) %>% 
  select(OID, SAKID, SDate, SDate.Yr, SDate.Mt, OID_IDate, WDate, WDate.Yr, WDate.Mt) -> 
  SPDCHRE2

# Examine SAKs for offenders w/ multiple SAKs at least one of which has missing SDate. 
as.data.frame(SPDCHRE2)

```

Next, we need to implement some decision rules about reconciling discrepancies 
between SAK collection date (SDate) and the CHR incident date (IDate) for SAKs
that were previously matched to CHR incident records. This is necessary so that
we can create a new variable called WDate to store the start date for the SAK 
testing window discussed above. 

```{r} 
SPDCHR %>% 
  # Retain only the earliest SAK (first record after the sort above) for each offender.
  #filter(row_number() == 1) ->
  SPDE
...
CON %>% 
  ...
  mutate(NCON = if_else(is.na(NCON), true = 0, false = NCON)) -> 
  IDN2
```
...
ARR %>% 
  ...
  summarize_at(.vars = avars, .funs = c("max")) %>% 
  group_by(OID) %>% 
  # Aggregate dummy codes by OID into incident count variables
  summarize_at(.vars = avars, .funs = c("sum")) ->
  ARR2
```
...
CHG %>% 
  group_by(OID, IID) %>% 
  # Aggregate dummy codes by OID + IID into binary variables
  summarize_at(.vars = cvars, .funs = c("max")) %>% 
  group_by(OID) %>% 
  # Aggregate dummy codes by OID into incident count variables
  summarize_at(.vars = cvars, .funs = c("sum")) ->
  CHG3
```
...
JUD %>% 
  group_by(OID, IID) %>% 
  # Aggregate dummy codes by OID + IID into binary variables
  summarize_at(.vars = jvars, .funs = c("max")) %>% 
  group_by(OID) %>% 
  # Aggregate dummy codes by OID into incident count variables
  summarize_at(.vars = jvars, .funs = c("sum")) ->
  JUD3
```
