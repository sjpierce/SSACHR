...
read_sav(here::here("inst/extdata/SSA_ARR_Arrests_Imputed_2018-09-03.sav"), 
  ...
  var_labels(ACat12_Any = "Arrested for any of 12 main crime categories") ->
  ARR
```
...
read_sav(here::here("inst/extdata/SSA_CHG_PA_Charges_Imputed_2018-09-03.sav"), 
  ...
  var_labels(CCat12_Any = "Charged for any of 12 main crime categories") ->
  CHG
```
...
read_sav(here::here("inst/extdata/SSA_JUD_Judicial_Charges_Imputed_2018-09-03.sav"), 
  ...
  var_labels(JCat12_Any = "Adjudicated for any of 12 main crime categories") ->
  JUD
```
...
JUD %>% 
  ...
  mutate(NCON = 1) %>% 
  # Add variable label. 
  var_labels(NCON = "Adjudicated charge record w/ conviction") ->
  CON
```
...
are not restricted to incidents involving arrests, charge, or conviction for one
of the 12 main crime categories. We will construct additional variables
...
identified or that were coded ACat12 = 999 (excluded user-missing) leaves 
`r nrow(ARREW)` arrest offense records in *ARREW*. Table 
...
identified or that were coded CCat12 = 999 (excluded user-missing) leaves 
`r nrow(CHGEW)` prosecutor charge records in *CHGEW*. Table
...
the records associated with offenders whose earliest testing window could not be
identified or that were coded JCat12 = 999 (excluded user-missing) leaves
`r nrow(JUDEW)` judicial charge records in *JUDEW*. Table
...
the records associated with offenders whose earliest testing window could not be
identified or that were coded JCat12 = 999 (excluded user-missing) leaves
`r nrow(CONEW)` judicial charge records in *CONEW*. Table
...
INCEW %>% 
  ...
  summarize(HXCat12_1 = sum(HXCat12_1),
            HXCat12_2 = sum(HXCat12_2),
            HXCat12_3 = sum(HXCat12_3),
            HXCat12_4 = sum(HXCat12_4),
            HXCat12_5 = sum(HXCat12_5),
            HXCat12_6 = sum(HXCat12_6),
            HXCat12_7 = sum(HXCat12_7),
            HXCat12_8 = sum(HXCat12_8),
            HXCat12_9 = sum(HXCat12_9),
            HXCat12_10 = sum(HXCat12_10),
            HXCat12_11 = sum(HXCat12_11),
            HXCat12_12 = sum(HXCat12_12),
            HXCat12_999 = sum(HXCat12_999)) %>% 
  # Merge count variables onto IDNEW.
  left_join(x = IDNEW, y = ., by = "OID") %>% 
  var_labels(HXCat12_1 = "No. incidents arrested, charged, or convicted for arson",
             ...
             HXCat12_5 = "No. incidents arrested, charged, or convicted for riminal sexual conduct",
             ...
             HXCat12_12 = "No. incidents arrested, charged, or convicted for eapons",
...
INCEW %>% 
  ...
  pivot_wider(names_from = IWhen, 
              values_from = c(HXCat12_1, HXCat12_2, HXCat12_3, HXCat12_4, 
                              HXCat12_5, HXCat12_6, HXCat12_7, HXCat12_8, 
                              HXCat12_9, HXCat12_10, HXCat12_11, HXCat12_12,
                              HXCat12_999)) %>% 
  # Merge count variables onto IDNEW.
  left_join(x = IDNEW, y = ., by = "OID") %>% 
