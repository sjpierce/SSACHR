...
INCEW %>% 
  ...
  var_labels(NINC_Before = paste(get_label(IDNEW$NINC), 
                                 "before earliest testing window"),
             NINC_During = paste(get_label(IDNEW$NINC), 
                                 "during earliest testing window"),
             NINC_After  = paste(get_label(IDNEW$NINC), 
                                 "after earliest testing window")) ->
  INCEWAgg
```
\FloatBarrier
...
ARREW %>% 
  ...
  var_labels(NARR_Before = paste(get_label(IDNEW$NARR), 
                                 "before earliest testing window"),
             NARR_During = paste(get_label(IDNEW$NARR), 
                                 "during earliest testing window"),
             NARR_After  = paste(get_label(IDNEW$NARR), 
                                 "after earliest testing window")) ->
  ARREWAgg
```
...
CHGEW %>% 
  ...
  var_labels(NCHG_Before = paste(get_label(IDNEW$NCHG), 
                                 "before earliest testing window"),
             NCHG_During = paste(get_label(IDNEW$NCHG), 
                                 "during earliest testing window"),
             NCHG_After  = paste(get_label(IDNEW$NCHG), 
                                 "after earliest testing window")) ->
  CHGEWAgg
```
...
JUDEW %>% 
  ...
  var_labels(NJUD_Before = paste(get_label(IDNEW$NJUD), 
                                 "before earliest testing window"),
             NJUD_During = paste(get_label(IDNEW$NJUD), 
                                 "during earliest testing window"),
             NJUD_After = paste(get_label(IDNEW$NJUD), 
                                 "after earliest testing window")) ->
  JUDEWAgg
```
...
CONEW %>% 
  ...
  var_labels(NCON_Before = paste(get_label(IDNEW$NCON), 
                                 "before earliest testing window"),
             NCON_During = paste(get_label(IDNEW$NCON), 
                                 "during earliest testing window"),
             NCON_After = paste(get_label(IDNEW$NCON), 
                                "after earliest testing window")) ->
  CONEWAgg
```
\FloatBarrier
...
The incident counts are then merged onto the *IDNEW* data frame via left_join()
because it preserves variable attributes better than merge() and is more
compatible with the tidyverse style and piped operator %>%. After merging, we 
replace the missing values in the new variables with zeroes, then compute some
additional variables as well. 

```{r update_IDNEW}
IDNEW %>% 
  # Merge in incident record count variables
  left_join(x = ., y = INCEWAgg, by = "OID") %>% 
  # Merge in arrest record count variables
  left_join(x = ., y = ARREWAgg, by = "OID") %>% 
  # Merge in charge record count variables
  left_join(x = ., y = CHGEWAgg, by = "OID") %>% 
  # Merge in judicial charge record count variables
  left_join(x = ., y = JUDEWAgg, by = "OID") %>% 
  # Merge in conviction record count variables
  left_join(x = ., y = CONEWAgg, by = "OID") %>% 
  # Merge in ACat12 incident count variables 
  left_join(x = ., y = ARR2, by = "OID") %>% 
  # Merge in CCat12 incident count variables 
  left_join(x = ., y = CHG2, by = "OID") %>% 
  # Merge in JCat12 incident count variables (for convictions only)
  left_join(x = ., y = CON2, by = "OID") %>% 
  # Replace all NA values in the new variables with 0. 
  mutate(across(.cols = all_of(acjvars), .fns = replace_na, replace = 0)) %>% 
...
bind_rows(dfsummary(IDN, label = "Offenders...Before", dvar = IDN$DOB),
          dfsummary(IDNEW, label = "Offenders...After", dvar = IDNEW2$DOB),
...
- We use [RStudio (version 1.3.1056 or later)](www.rstudio.org) to work with 
...
- We used [pandoc](https://pandoc.org) version `r pandoc_version()` for this 
  document. 
