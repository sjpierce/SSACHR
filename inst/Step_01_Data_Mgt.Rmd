...
all(IDNEW2$OCSC1 == IDNEW2$ACat12_5) # Arrest incident counts
all(IDNEW2$OCSC2 == IDNEW2$CCat12_5) # Charge incident counts
all(IDNEW2$OCSC3 == IDNEW2$JCat12_5) # Conviction incident counts
```
...
# Summarize record counts before case selection for this study.
bind_rows(# Before case selection
          dfsummary(IDN, label = "Offenders", dvar = IDN$DOB),
          ...
          dfsummary(IDNEW2, label = "Offenders", dvar = IDN$DOB),
          dfsummary(INCEW, label = "Incidents", dvar = INC$IDate), 
          dfsummary(ARREW, label = "Arrest offenses", dvar = ARR$ADate),
          dfsummary(CHGEW, label = "Prosecutor charges", dvar = CHG$CDate),
          dfsummary(JUDEW, label = "Adjudicated charges (all dispositions)", 
                    dvar = JUD$JDate),
          dfsummary(CONEW, label = "Adjudicated charges (convictions)", 
                    dvar = CON$JDate)) ->
  CHR.RC
...
```{r}
CCC <- describe(IDNEW2[, c("ACat12_Count", "CCat12_Count", "JCat12_Count")],
                quant=c(.25, .50, .75))
...
kable(freq(IDNEW2$ACat12_Count), digits = 2, 
      caption = "Crime Category Count (With Arrests)")

kable(freq(IDNEW2$CCat12_Count), digits = 2,
      caption = "Crime Category Count (With Charges)")

kable(freq(IDNEW2$JCat12_Count), digits = 2,
