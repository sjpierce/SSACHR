...
each offender and store that as a new dataset. We'll need to tack on the  

```{r}
SPD %>% 
  group_by(OID) -> 
  # Retain only the earliest SAK 9first record after the sort above) for each offender.
  #filter(row_number() == 1) ->
  SPDE
