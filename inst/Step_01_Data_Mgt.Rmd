...
* **Tibble.** The tidyverse group of R packages use a modified data frames 
  called tibbles. It is still basically a tabular data set comprised of rows
  (observations or cases) and columns (variables). 
...
variables while importing data from the SPSS data files into the *ARREW*,
*CHGEW*, and *CONEW* tibbles. In the *ARREW* tibble, the variables 
*ACat12_1* to *ACat12_12* and *ACat12_999* mark which category of crime the 
offender was arrested for on a given arrest offense record. The corresponding 
*CCat12_1* to *CCat12_12* and *CCat12_999* variables in *CHGEW* show which 
crime category the individual was charged with by a prosecuting attorney on a 
charge record. Meanwhile, *JCat12_1* to *JCat12_12* and *JCat12_999* variables 
in *CONEW* show which crime category the offender was convicted of on a given 
conviction record. 

All three of those tibbles can have multiple records per criminal incident. This
section aggregates those dummy coded crime
category variables from the record level to the incident level to flag presence
or absence of each crime category on the incident, then merges the resulting
variables onto the *INCEW* tibble. That allows us to see which incidents were 
associated with arrests, charges, and/or convictions for each category of crime. 
...
```{r create_ARREW_Agg_IID}
# Aggregate ACat12 dummy codes by OID + IID to incident level binary variables.
ARREW %>% 
  group_by(OID, IID) %>% 
  summarize(across(.cols = all_of(avars), .fns = max)) ->
  ARREW_Agg_ACat12_IID
```
...
```{r create_CHGEW_Agg_IID}         
# Aggregate CCat12 dummy codes by OID + IID to incident level binary variables.
CHGEW %>% 
  group_by(OID, IID) %>% 
...
```{r create_CONEW_Agg_IID}
# Aggregate JCat12 dummy codes by OID + IID to incident level binary variables.
CONEW %>% 
  group_by(OID, IID) %>% 
...
IDNEW %>% 
  ...
  left_join(x = ., y = ARREW_Agg_OID, by = "OID") %>% 
  # Merge in CCat12 incident count variables 
  left_join(x = ., y = CHGEW_Agg_OID, by = "OID") %>% 
  # Merge in JCat12 incident count variables (for convictions only)
  left_join(x = ., y = CONEW_Agg_OID, by = "OID") %>% 
