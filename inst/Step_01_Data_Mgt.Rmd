...
information they don’t have yet. Adopting a testing window definition based on 
...
also merge these variables into *IDNEW*. 
...
ARREW %>% 
  ...
  left_join(x = IDNEW, y = ., by = "OID") %>% 
  # Replace all NA values in the new variables with 0. 
  mutate(across(.cols = all_of(c("NARR_Before", "NARR_During", "NARR_After")), 
                .fns = replace_na, replace = 0)) ->
  IDNEW
```
...
can aggregate as follows to get them. We merge these variables into *IDNEW*
too.
...
CHGEW %>% 
  ...
  left_join(x = IDNEW, y = ., by = "OID") %>% 
  # Replace all NA values in the new variables with 0. 
  mutate(across(.cols = all_of(c("NCHG_Before", "NCHG_During", "NCHG_After")), 
                .fns = replace_na, replace = 0)) ->
  IDNEW
```
...
JUDEW %>% 
  ...
  left_join(x = IDNEW, y = ., by = "OID") %>% 
  # Replace all NA values in the new variables with 0. 
  mutate(across(.cols = all_of(c("NJUD_Before", "NJUD_During", "NJUD_After")), 
                .fns = replace_na, replace = 0)) ->
  IDNEW
```
...
CONEW %>% 
  ...
  left_join(x = IDNEW, y = ., by = "OID") %>% 
  # Replace all NA values in the new variables with 0. 
  mutate(across(.cols = all_of(c("NCON_Before", "NCON_During", "NCON_After")), 
                .fns = replace_na, replace = 0)) ->
  IDNEW
```
\FloatBarrier
...
and/or convictions for each category of crime. 
...
hxvars     <- paste("HXCat12", c(1:12, 999), sep = "_")
...
CrimeCats <- data.frame(Label = get_labels(ARREW$ACat12),
                        ARREW = avars,
                        CHGEW = cvars,
                        CONEW = jvars)
...
ACat12_12 and ACat12_999 each mark whether or not a particular arrest offense
record was associated with a specific type of crime. Here, we aggregate them up
to the incident level by taking the maximum observed value for each incident.
The resulting incident-level records will have values of 1 for every crime
category for which the offender was arrested due to that incident and values of
0 for every crime category for which the offender was not arrested due to that
incident.
...
For any given incident (identifed by a unique value of *IID*), there can be
multiple prosecutor charges records in *CHGEW*. The dummy variables CCat12_1 to
CCat12_12 and CCat12_999 each mark whether or not a particular prosecutior charge 
record was associated with a specific type of crime. Here, we aggregate them up
to the incident level by taking the maximum observed value for each incident.
The resulting incident-level records will have values of 1 for every crime
category for which the offender was charged due to that incident and values of
0 for every crime category for which the offender was not charged due to that
incident.
...
conviction charge record was associated with a specific type of crime. Here, we
aggregate them up to the incident level by taking the maximum observed value for
each incident. The resulting incident-level records will have values of 1 for
every crime category for which the offender was convicted  due to that incident
and values of 0 for every crime category for which the offender was not
convicted due to that incident.
...
# Table caption.
TCap <- paste("Crosstabulate Incident-Level Criminal Sexual Conduct History",
              "Variable HXCat12_5 With CSC_ANY")
# Footnote text.
...
xtabs(~HXCat12_5 + CSC_ANY, data = INCEW) %>% 
  as.data.frame() %>% 
  mutate(Percent = 100*Freq/nrow(INCEW)) %>% 
  kable(., format = "html", booktabs = TRUE, caption = TCap, digits = 2,
        col.names = clabels, format.args = list(big.mark = ",")) %>% 
  column_spec(1:3, width = "1 cm") %>% 
  footnote(general = FN, general_title = "Note: ", footnote_as_chunk = TRUE,
         threeparttable = TRUE)
```

# Aggregate Crime Category History Variables
Having created the crime category history variables at the incident level, next
we aggregate them to offender-level incident count varaiables named *NHXCat12_1* 
to *NHXCat12_12* and *NHXCat12_999*. Each of these will cntain the number of 
...
INCEW %>% 
  ...
  # Aggregate by OID to get count variables (overall)
  summarize(NHXCat12_1 = sum(HXCat12_1),
            NHXCat12_2 = sum(HXCat12_2),
            NHXCat12_3 = sum(HXCat12_3),
            NHXCat12_4 = sum(HXCat12_4),
            NHXCat12_5 = sum(HXCat12_5),
            NHXCat12_6 = sum(HXCat12_6),
            NHXCat12_7 = sum(HXCat12_7),
            NHXCat12_8 = sum(HXCat12_8),
            NHXCat12_9 = sum(HXCat12_9),
            NHXCat12_10 = sum(HXCat12_10),
            NHXCat12_11 = sum(HXCat12_11),
            NHXCat12_12 = sum(HXCat12_12),
            NHXCat12_999 = sum(HXCat12_999)) %>% 
  # Merge count variables onto IDNEW.
  left_join(x = IDNEW, y = ., by = "OID") %>% 
  var_labels(NHXCat12_1 = "No. incidents arrested, charged, or convicted for arson",
             NHXCat12_2 = "No. incidents arrested, charged, or convicted for assault - DV, stalking",
             NHXCat12_3 = "No. incidents arrested, charged, or convicted for assault - non-sexual, non-DV",
             NHXCat12_4 = "No. incidents arrested, charged, or convicted for burglary",
             NHXCat12_5 = "No. incidents arrested, charged, or convicted for riminal sexual conduct",
             NHXCat12_6 = "No. incidents arrested, charged, or convicted for drug crime",
             NHXCat12_7 = "No. incidents arrested, charged, or convicted for homicide",
             NHXCat12_8 = "No. incidents arrested, charged, or convicted for larceny/theft/fraud",
             NHXCat12_9 = "No. incidents arrested, charged, or convicted for robbery",
             NHXCat12_10 = "No. incidents arrested, charged, or convicted for sex crime - other, excluding CSC",
             NHXCat12_11 = "No. incidents arrested, charged, or convicted for traffic/ordinances",
             NHXCat12_12 = "No. incidents arrested, charged, or convicted for eapons",
             NHXCat12_999 = "No. incidents arrested, charged, or convicted for excluded user-missing") ->
  IDNEW
```
...
INCEW %>% 
  ...
  summarize(NHXCat12_1 = sum(HXCat12_1),
            ...
            NHXCat12_12 = sum(HXCat12_12),
            NHXCat12_999 = sum(HXCat12_999)) %>% 
  pivot_wider(names_from = IWhen, 
              values_from = c(NHXCat12_1, NHXCat12_2, NHXCat12_3, NHXCat12_4, 
                              NHXCat12_5, NHXCat12_6, NHXCat12_7, NHXCat12_8, 
                              NHXCat12_9, NHXCat12_10, NHXCat12_11, NHXCat12_12,
                              NHXCat12_999)) %>% 
  # Merge count variables onto IDNEW.
  left_join(x = IDNEW, y = ., by = "OID") %>% 
  var_labels(NHXCat12_1_Before = "No. incidents arrested, charged, or convicted for arson before testing window",
             NHXCat12_2_Before = "No. incidents arrested, charged, or convicted for assault - DV, stalking before testing window",
             NHXCat12_3_Before = "No. incidents arrested, charged, or convicted for assault - non-sexual, non-DV before testing window ",
             NHXCat12_4_Before = "No. incidents arrested, charged, or convicted for burglary before testing window",
             NHXCat12_5_Before = "No. incidents arrested, charged, or convicted for criminal sexual conduct before testing window",
             NHXCat12_6_Before = "No. incidents arrested, charged, or convicted for drug crime before testing window",
             NHXCat12_7_Before = "No. incidents arrested, charged, or convicted for homicide before testing window",
             NHXCat12_8_Before = "No. incidents arrested, charged, or convicted for larceny/theft/fraud before testing window",
             NHXCat12_9_Before = "No. incidents arrested, charged, or convicted for robbery before testing window",
             NHXCat12_10_Before = "No. incidents arrested, charged, or convicted for sex crime - other, excluding CSC before testing window",
             NHXCat12_11_Before = "No. incidents arrested, charged, or convicted for traffic/ordinances before testing window",
             NHXCat12_12_Before = "No. incidents arrested, charged, or convicted for weapons before testing window",
             NHXCat12_999_Before = "No. incidents arrested, charged, or convicted for excluded user-missing before testing window",
             NHXCat12_1_During = "No. incidents arrested, charged, or convicted for arson during testing window",
             NHXCat12_2_During = "No. incidents arrested, charged, or convicted for assault - DV, stalking during testing window",
             NHXCat12_3_During = "No. incidents arrested, charged, or convicted for assault - non-sexual, non-DV during testing window",
             NHXCat12_4_During = "No. incidents arrested, charged, or convicted for burglary during testing window",
             NHXCat12_5_During = "No. incidents arrested, charged, or convicted for criminal sexual conduct during testing window",
             NHXCat12_6_During = "No. incidents arrested, charged, or convicted for drug crime during testing window",
             NHXCat12_7_During = "No. incidents arrested, charged, or convicted for homicide during testing window",
             NHXCat12_8_During = "No. incidents arrested, charged, or convicted for larceny/theft/fraud during testing window",
             NHXCat12_9_During = "No. incidents arrested, charged, or convicted for robbery during testing window",
             NHXCat12_10_During = "No. incidents arrested, charged, or convicted for sex crime - other, excluding CSC during testing window",
             NHXCat12_11_During = "No. incidents arrested, charged, or convicted for traffic/ordinances during testing window",
             NHXCat12_12_During = "No. incidents arrested, charged, or convicted for weapons during testing window",
             NHXCat12_999_During = "No. incidents arrested, charged, or convicted for excluded user-missing during testing window",
             NHXCat12_1_After = "No. incidents arrested, charged, or convicted for arson after testing window",
             NHXCat12_2_After = "No. incidents arrested, charged, or convicted for assault - DV, stalking after testing window",
             NHXCat12_3_After = "No. incidents arrested, charged, or convicted for assault - non-sexual, non-DV after testing window",
             NHXCat12_4_After = "No. incidents arrested, charged, or convicted for burglary after testing window",
             NHXCat12_5_After = "No. incidents arrested, charged, or convicted for criminal sexual conduct after testing window",
             NHXCat12_6_After = "No. incidents arrested, charged, or convicted for drug crime after testing window",
             NHXCat12_7_After = "No. incidents arrested, charged, or convicted for homicide after testing window",
             NHXCat12_8_After = "No. incidents arrested, charged, or convicted for larceny/theft/fraud after testing window",
             NHXCat12_9_After = "No. incidents arrested, charged, or convicted for robbery after testing window",
             NHXCat12_10_After = "No. incidents arrested, charged, or convicted for sex crime - other, excluding CSC after testing window",
             NHXCat12_11_After = "No. incidents arrested, charged, or convicted for traffic/ordinances after testing window",
             NHXCat12_12_After = "No. incidents arrested, charged, or convicted for weapons after testing window",
             NHXCat12_999_After = "No. incidents arrested, charged, or convicted for excluded user-missing after testing window") ->
  IDNEW
```
...
IDNEW %>% 
  ...
  mutate(ACat12_Count = sum(c_across(all_of(avars[1:12])) > 0),
         ...
         HXCat12_Count_Before = sum(c_across(all_of(hvxarsb[1:12])) > 0),
