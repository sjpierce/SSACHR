...
### Merge Testing Window Start Date Onto Incident Data
The next step is to merge variables from *SPDCHREW* onto the incident data in 
*INC* so that we can classify each incident according to whether it occurred 
before, during, or after the earliest testing window for the offender. Along the
way we subset down to the records that have non-missing WDate values and save to
a new tibble called *INCEW*. 
...
SPDCHREW %>% 
  ...
  select(SAKID, OID, OID_IID, WDate, TWindow) %>% 
  ...
  mutate(# Classify incidents as before, during, or after the testing window. 
         ...
         # Convert to a factor with levels in sensible order for display.
         IWhen = factor(IWhen, levels = IWhenLevels)) %>% 
...
matching the *OID_IID* variable merged in from the SPDCHREW tibble. Such
...
*WDate* ad *TWindow*. Table \ref{tab:examine_IDate} shows that there are only a
...
```{r examine_IDate}
# Table caption.
T2cap <- paste("Crosstabulation of Whether Incident Date Was Within Testing",
               "Window and Whether Incident Matched to Perpetrator's Earliest",
               "SAK")
# Footnote text.
FN2 <- paste("Cells contain counts of incidents. Only incidents with valid",
             "testing window start dates were included. ESAK, earliest sexual",
             "assaut kit; IDate, incident date; TWindow, testing window.")

# Vector of text values for column labels to be used in the table.
clabels <- c("IDate in TWindow", "FALSE", "TRUE", "Sum")

xtabs(~IDate %within% TWindow + (OID_IID == IID), addNA = TRUE, data = INCEW) %>% 
  addmargins() %>% 
  as.data.frame() %>% 
  pivot_wider(names_from = OID_IID....IID, values_from = Freq) %>% 
  kable(., format = "latex", booktabs = TRUE, caption = T2cap, 
        col.names = clabels, format.args = list(big.mark = ",")) %>% 
