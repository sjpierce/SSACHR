...
IDNEW %>%
  select(HXCat12_1_After) %>%
  filter(HXCat12_1_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_1_After)) %>%
  mutate(Category = "Arson",
         Group = "Property Crimes",
         SortOrder = 8) ->
  HCA_1

IDNEW %>%
  select(HXCat12_2_After) %>%
  filter(HXCat12_2_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_2_After)) %>%
  mutate(Category = "Assault, Domestic Violence",
         Group = "Violent Non-Sexual Crimes",
         SortOrder = 3) ->
  HCA_2

IDNEW %>%
  select(HXCat12_3_After) %>%
  filter(HXCat12_3_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_3_After)) %>%
  mutate(Category = "Assault, non-Domestic Violence",
         Group = "Violent Non-Sexual Crimes",
         SortOrder = 4) ->
  HCA_3

IDNEW %>%
  select(HXCat12_4_After) %>%
  filter(HXCat12_4_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_4_After)) %>%
  mutate(Category = "Burglary",
         Group = "Property Crimes",
         SortOrder = 9) ->
  HCA_4

# IDNEW %>%
#   select(HXCat12_5_After) %>%
#   filter(HXCat12_5_After > 0) %>%
#   ungroup() %>%
#   summarise(N = n(),
#             Pct = 100*N/nrow(IDNEW),
#             Incidents = sum(HXCat12_5_After),
#             Mean = mean(HXCat12_5_After),
#             Min = min(HXCat12_5_After),
#             Q25 = quantile(HXCat12_5_After, probs = 0.25),
#             Q50 = quantile(HXCat12_5_After, probs = 0.50),
#             Q75 = quantile(HXCat12_5_After, probs = 0.75),
#             Max = max(HXCat12_5_After))  %>%
#   mutate(Category = "Criminal Sexual Conduct (CSC)",
#          Group = "Sexual Crimes",
#          SortOrder = 1) ->
#   HCA_5
...
IDNEW %>%
  select(HXCat12_6_After) %>%
  filter(HXCat12_6_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_6_After))  %>%
  mutate(Category = "Drug Crimes",
         Group = "Other Crimes",
         SortOrder = 11) ->
  HCA_6

IDNEW %>%
  select(HXCat12_7_After) %>%
  filter(HXCat12_7_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_7_After))  %>%
  mutate(Category = "Homicide",
         Group = "Violent Non-Sexual Crimes",
         SortOrder = 5) ->
  HCA_7

IDNEW %>%
  select(HXCat12_8_After) %>%
  filter(HXCat12_8_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_8_After))  %>%
  mutate(Category = "Larceny, Theft, Fraud",
         Group = "Property Crimes",
         SortOrder = 10) ->
  HCA_8

IDNEW %>%
  select(HXCat12_9_After) %>%
  filter(HXCat12_9_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_9_After))  %>%
  mutate(Category = "Robbery",
         Group = "Violent Non-Sexual Crimes",
         SortOrder = 6) ->
  HCA_9

IDNEW %>%
  select(HXCat12_10_After) %>%
  filter(HXCat12_10_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_10_After)) %>%
  mutate(Category = "Sex Crimes (non-CSC crimes)",
         Group = "Sexual Crimes",
         SortOrder = 2) ->
  HCA_10

IDNEW %>%
  select(HXCat12_11_After) %>%
  filter(HXCat12_11_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_11_After)) %>%
  mutate(Category = "Traffic & Ordinances",
         Group = "Other Crimes",
         SortOrder = 12) ->
  HCA_11

IDNEW %>%
  select(HXCat12_12_After) %>%
  filter(HXCat12_12_After > 0) %>%
  ungroup() %>%
  summarise(N = n(),
            Pct = 100*N/nrow(IDNEW),
            Incidents = sum(HXCat12_12_After)) %>%
  mutate(Category = "Weapons",
         Group = "Violent Non-Sexual Crimes",
         SortOrder = 7) ->
  HCA_12
