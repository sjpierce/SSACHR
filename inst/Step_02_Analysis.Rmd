...
# Manage Data
The chunk below reorganizes the *IDNEW* data to a long format with three rows 
per offender (one row for each period before, during, and after the testing 
window) and stores it as *IDNEWL*. We will use this later for running GEE 
models. 
...
IDNEW %>% 
  ...
  pivot_longer(cols = -OID, names_to = c(".value", "When"), 
               names_pattern = "(.*)_(.*)",
               values_to = c("Duration", "HXCat12_Count", 
...
```{r cccvar_texreg, results = "asis"}
TCap <- paste("Generalized Estimating Equation Models for Crime Category Count",
              "Predicted by When Period Was Observed Relative to the Testing",
              "Window")
FN <- paste ("\\item Note.",
...
texreg(list(cccvar_gee1, cccvar_gee2), booktabs = TRUE, dcolumn = TRUE, threeparttable = TRUE,
       table = FALSE, use.packages = FALSE, single.row = TRUE, 
       stars = 0, ci.force = FALSE, caption = TCap, custom.note = FN)
```

```{r cccvar_emmeans}
# Table caption.
TCap <- paste("Estimated Marginal Means for Crime Category Counts By When",
              "Incidents Occurred")

# Table footnote.
FN <- paste("Model 1 did not use an offset.",
            "Model 2 used log(Years) as an offset term.")
...
as_tibble(cccvar_emmeans1$emmeans) %>% 
  ...
  kable(format = "latex", caption = TCap, 
        col.names = c("When", "Rate", "SE", "df", "LCL", "UCL")) %>% 
  kable_styling() %>% 
  add_header_above(c(" " = 4, "95% CI" = 2)) %>% 
  group_rows(group_label = "Model 1: Mean per offender (counts)", start_row = 1,
             end_row = 3, italic = TRUE) %>% 
  group_rows(group_label = "Model 2: Mean per offender-year (annual incidence rates)", 
             start_row = 4, end_row = 6, italic = TRUE) %>% 
  footnote(general = FN, general_title = "Note: ", footnote_as_chunk = TRUE,
         threeparttable = TRUE)
```

```{r cccvar_ratios, eval=FALSE}
cccvar_emmeans1$contrasts %>% 
  confint() %>%
  kable(format = "latex")
```
