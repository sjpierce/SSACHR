...
## Sex
```{r examine_Sex}
kable(freq(as_factor(IDNEW$Sex)),  digits = 2, caption = "Offender Sex")
```
...
## Race
```{r examine_Race}
kable(freq(as_factor(IDNEW$Race), user.missing = "Unknown"),  digits = 2, 
...
RC <- describe(IDNEW[, c("NINC", "NARR", "NCHG", "NJUD", "NCON")],  
               quant=c(.25, .50, .75))
...
```{r}
kable(freq(IDNEW$ACat12_Count), digits = 2, 
      caption = "Crime Category Count (With Arrests)")
```

```{r}
kable(freq(IDNEW$CCat12_Count), digits = 2,
      caption = "Crime Category Count (With Charges)")
```

```{r}
kable(freq(IDNEW$JCat12_Count), digits = 2,
