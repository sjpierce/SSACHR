...
offender’s testing window are linked to the very SAK that started the testing
window itself.
...
to see for the majority of the data (nearly values are for 12 or fewer
...
harder to see for the majority of the data (nearly values are for 15 or fewer
incidents), we also produced Figure \ref{fig:plot_BCrime_partial_width}, which
zooms in on the 0-15 range of incident counts.
...
Figure \ref{fig:plot_IYearsAfterTW_Crime} shows the distributions for the number
of years elapsed between the end of the testing window and each incident that
occurred after the window ended broken down by crime category.
...
Figure \ref{fig:plot_IYearsAfterTW_Crime2} shows the cumulative distribution 
functions for the number of years elapsed between the end of the testing window
and and each incident that occurred after the window ended broken down by 
broader crime category. This is useful because the number of preventable 
incidents is basically dependent on when after the end of the testing window the
offender reference sample gets uploaded to CODIS. 

```{r plot_IYearsAfterTW_Crime2, fig.height = 6, fig.width = 7.2, fig.cap = FCap}
FCap <- paste("\\label{fig:plot_IYearsAfterTW_Crime2}",
...
Figure \ref{fig:plot_IYearsAfterTW_Crime} and use them to prepare an 
additional table. A reviewer noted that uploading an offender DNA profile from a
SAK to CODIS can only produce immediate identification of the offender if the 
corresponding offender reference sample was already present in CODIS beforehand. 
However, that may not always happen. The offender reference sample that allows 
identifying the offender associated with SAK may not arrive in CODIS until 
later. That means there could be a delay between uploading the SAK forensic 
testing results to CODIS as a forensic sample and getting a match to an 
offender. Incidents occurring between the end of the testing window and the 
arrival of a matching offender reference sample cannot be prevented through
...
offender match. There would be no delay (0 years) when the offender reference 
sample was already present in CODIS, but it could be anywhere from 1 day to many
years before one is available. Knowing more about that distribution would allow
us to find the subset of potentially preventable incidents that occurred both 
after the testing window ended and after the matching reference sample was 
uploaded to CODIS. 
...
graphed in Figure \ref{fig:plot_IYearsAfterTW_Crime2} to compute how many 
...
INCEWA %>% 
  ...
  kable(format = "latex", digits = 1, booktabs = TRUE, caption = TCap, 
  ...
  add_header_above(., c(" ", " ", "Delay in Matching to Offender Reference Sample" = 8)) %>% 
  row_spec(., row = c(1, 4, 10, 14), italic = TRUE) %>% 
  pack_rows(" ", start_row = 1, end_row = 3) %>% 
  pack_rows(" ", start_row = 4, end_row = 9) %>% 
  pack_rows(" ", start_row = 10, end_row = 13) %>% 
...
We still need to fine-tune the plotting and export to get nice figures. That may
require eliminating transparency (alpha) and deciding between TIFF versus EPS 
files. The EPS files are smaller. I may also need to put the boxplots in before
the points.

```{r explort_Figure_Crime_Counts}
ggsave(filename = here::here("inst/Figure_Crime_Counts.eps"), 
       plot = Figure_Crime_partial_width, device = "eps", dpi = 600, 
       height = 6.0, width = 7.2, units = "in")
```

```{r explort_Figure_Crime_Years}
