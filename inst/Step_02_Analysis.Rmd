...
# Rearrange data so we can easily plot multiple variables. 
cscvars  <- c("ACat12_5", "CCat12_5", "JCat12_5")
csctypes <- c("Arrests", "Prosecutor Charges", "Convictions")

IDNEW %>% 
  select(OID, ACat12_5, CCat12_5, JCat12_5) %>% 
  # Rearrange data so we can easily plot multiple variables. 
  pivot_longer(cols = -OID, names_to = "VName", values_to = "Count") %>%
...
densityplot(~ Count | VName, data = CSCCOUNTS, layout = c(3,1), 
