...
range of values for this count is 0-12. We report both overall results, and 
...
Here we summarize the counts of unique incidents where the offender was 
*arrested for*, *charged with*, and *convicted of* arson. We also include 
counts of incidents based on a combined criterion where the offender was 
*arrested, charged, or convicted* (any of the three, or any combination of them)
of arson. 

```{r describe_arsonvars}
# Table caption. 
TCap <- paste("Offender-Level Arson Incidents (Overall and By When Incident",
              "Occurred)")
# Footnote text. 
FN <- paste("Only offenders with valid testing window start dates are included.",
            "Before, during, and after refer to when the incident associated",
...
TRowLabs <- c("Arson incident count (arrested)", BDA, 
              "Arson incident count (charged)", BDA,
              "Arson incident count (convicted)", BDA,
              "Arson incident count (arrested, charged, or convicted)", BDA)
...
IDNEW %>% 
  select(ACat12_1, ACat12_1_Before, ACat12_1_During, ACat12_1_After, 
         ...
         NArsonHistory, NArsonHistory_Before, NArsonHistory_During, 
         NArsonHistory_After) %>% 
  describe(., quant=c(.25, .50, .75)) %>% 
  bind_cols(data.frame(Variable = csclabs), .) %>% 
  select(Variable, n, mean, sd, min, max, skew, kurtosis, Q0.25, Q0.5, Q0.75) %>% 
  kable(., format = "latex", booktabs = TRUE, digits = 2, row.names = FALSE, 
        col.names = c("Variable", CNames), caption = TCap,
...
FN <- paste0("N = ", format(nrow(IDNEW), big.mark = ","), ". TW, earliest ",
            ...
            "An incident is counted as showing an arson history if there are ",
            "any arson arrest, charge, or conviction records associated with ",
            "it (any one type, or any combination of them will suffice).")

# Get frequency distributions. 
IDNEW %>% 
  select(OID, NArsonHistory, NArsonHistory_Before, NArsonHistory_During, 
               NArsonHistory_After) %>% 
  rename(All = NArsonHistory, Before = NArsonHistory_Before, 
         During = NArsonHistory_During, After = NArsonHistory_After) %>% 
  pivot_longer(cols = -OID, names_to = "When", values_to = "Value") %>% 
  group_by(When, Value) %>% 
  count() %>% 
  arrange(Value) %>% 
...
```{r create_ArsonCOUNTS, fig.height = 3, warning= FALSE}
# Rearrange data so we can easily plot multiple variables. 
arsonvars  <- c("ACat12_1", "CCat12_1", "JCat12_1", "NArsonHistory")
arsontypes <- c("Arrested", "Charged", "Convicted", "Arrested, Charged, or Convicted")

IDNEW %>% 
  select(OID, ACat12_1, CCat12_1, JCat12_1, NArsonHistory) %>% 
  # Rearrange data so we can easily plot multiple variables. 
  pivot_longer(cols = -OID, names_to = "VName", values_to = "Count") %>%
  mutate(VName = ordered(VName, levels = cscvars, labels = csctypes)) ->
  ArsonCOUNTS
```  

```{r densityplot_ArsonCOUNTS, fig.height = 3, fig.width = 8.5, fig.cap=FCap}
# Figure caption.
...
densityplot(~ Count | VName, data = ArsonCOUNTS, layout = c(4,1), 
...
csclabs <- c("CSC incident count (arrested)", BDA, 
...
IDNEW %>% 
  select(ACat12_5, ACat12_5_Before, ACat12_5_During, ACat12_5_After, 
         ...
         NCSCHistory, NCSCHistory_Before, NCSCHistory_During, 
         NCSCHistory_After) %>% 
  describe(., quant=c(.25, .50, .75)) %>% 
  bind_cols(data.frame(Variable = csclabs), .) %>% 
  select(Variable, n, mean, sd, min, max, skew, kurtosis, Q0.25, Q0.5, Q0.75) %>% 
...
# Get frequency distributions. 
IDNEW %>% 
  select(OID, NCSCHistory, NCSCHistory_Before, NCSCHistory_During, 
               NCSCHistory_After) %>% 
  rename(All = NCSCHistory, Before = NCSCHistory_Before, 
         During = NCSCHistory_During, After = NCSCHistory_After) %>% 
  pivot_longer(cols = -OID, names_to = "When", values_to = "Value") %>% 
  group_by(When, Value) %>% 
  count() %>% 
  arrange(Value) %>% 
...
# Rearrange data so we can easily plot multiple variables. 
cscvars  <- c("ACat12_5", "CCat12_5", "JCat12_5", "NCSCHistory")
csctypes <- c("Arrested", "Charged", "Convicted", "Arrested, Charged, or Convicted")

IDNEW %>% 
  select(OID, ACat12_5, CCat12_5, JCat12_5, NCSCHistory) %>% 
  # Rearrange data so we can easily plot multiple variables. 
  pivot_longer(cols = -OID, names_to = "VName", values_to = "Count") %>%
  mutate(VName = ordered(VName, levels = cscvars, labels = csctypes)) ->
  CSCCOUNTS
