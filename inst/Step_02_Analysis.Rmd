...
# Footnote text.
FN <- paste0("Only incidents where the offender was arrested for, charged with, ",
...
IDNEW %>% 
  ...
  rename(All = NAR12R, Before = NARR12_Before, During = NARR12_During, 
         After = NARR12_After) %>% 
  pivot_longer(cols = -OID, names_to = "When", values_to = "Value") %>% 
  group_by(When, Value) %>% 
  count() %>% 
...
of crime each offender was *arrested for, charged with, or convicted of*
according to the CHR data. This sums a set of binary transforms on the incident
count variables associated with each crime category in *HXCat12*. The 
...
Below, Table \ref{tab:freq_HXCat12_Count} shows the frequency distributions 
underlying the summaries in Table \ref{tab:describe_cccvars}. 
