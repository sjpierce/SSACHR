...
FN <- paste("Separately for each crime category, we counted offenders",
            ...
            "The numerator for each percentage is the listed value for n; the",
            "denominator is the full sample size (N = 1,082).",
            "Incidents associated with more than one kind of crime are counted",
            "in all applicable specific categories, but only once per",
            "applicable broader category. For example, an incident with both a",
            "homicide and a robbery only counts once toward the total incident",
            "count for the broader violent crimes category. TW, testing",
            "window.")
...
IDNEWA %>%
  ...
  geom_boxplot(lwd = 1, width = .5, outlier.shape = NA, alpha = .1) +
  ...
  HXCat12_After_Crime_Distributions
...
IDNEWA %>%
  ...
  scale_y_continuous(limits = c(-.65, 1.00), 
                     breaks = c(0, .5, 1.0), labels = c("0%", "50%", "100%")) ->
  HXCat12_After_BCrime_Distributions
...
ggsave(filename = "Figure_Crime_Counts.tiff", plot = Figure_Crime_Counts, 
       device = "tiff", dpi = 600, height = 6, width = 7.2, units = "in")
ggsave(filename = "Figure_Crime_Counts_Flattened.eps", plot = Figure_Crime_Counts, 
       device = cairo_ps, dpi = 600, height = 6, width = 7.2, units = "in",
       fallback_resolution = 600)
ggsave(filename = "Figure_Crime_Counts.eps", plot = Figure_Crime_Counts, 
       device = "eps", dpi = 600, height = 6, width = 7.2, units = "in")
```

```{r explort_Figure_BCrime_Counts}
ggsave(filename = "Figure_BCrime_Counts.tiff", plot = Figure_BCrime_Counts, 
       device = "tiff", dpi = 600, height = 6, width = 7.2, units = "in")
ggsave(filename = "Figure_BCrime_Counts_Flattened.eps", plot = Figure_BCrime_Counts, 
       device = cairo_ps, dpi = 600, height = 6, width = 7.2, units = "in",
       fallback_resolution = 600)
ggsave(filename = "Figure_BCrime_Counts.eps", plot = Figure_BCrime_Counts, 
       device = "eps", dpi = 600, height = 6, width = 7.2, units = "in")
```
...
BCLevels <- c("HXCat12_Sexual", "HXCat12_Violent", "HXCat12_Property", 
              "HXCat12_Other")
...
# Graph after period frequency distributions.
INCEW %>%
  # Keep only the violent crimes.
  filter(IWhen == "After") %>%
  pivot_longer(cols = c(HXCat12_Sexual, HXCat12_Violent, HXCat12_Property, 
                        HXCat12_Other), 
               names_to = "BCrime",
               values_to = "Include") %>% 
  mutate(BCrime = factor(BCrime, levels = BCLevels, labels = BCLabels)) %>% 
  filter(Include == 1) %>% 
  select(OID, IID, BCrime, IYearsAfterTW) %>% 
  # use y = -... to position boxplot and jitterplot below the histogram
  ggplot(data = ., aes(x = IYearsAfterTW, y = -.4)) +
  # Add density plot with bar heights scaled to percentages via after_stat.
  ggdist::stat_halfeye(slab_fill = "black", width = .6, justification = -.42, 
                       .width = 0, point_colour = "NA") +
  # Add a jittered strip plot. 
  geom_point(aes(), position = position_jitter(width = 0, height = .25),
             size = 1, alpha = 0.5, color = "gray50") +
  # Overlay a boxplot on the strip plot from ggstance
  geom_boxplot(lwd = 1, width = .50, outlier.shape = NA, alpha = .1) +
  # Merged those calls to one display
  guides(scale = "none") +
  # Split each crime category into a separate panel (facet)
  facet_wrap(~BCrime, nrow = 4, strip.position = "top") +
  xlab("Years Elapsed Between End of Testing Window and Incident Date") +
  ylab("Density") +
  # Remove y axis title label from the y axis
