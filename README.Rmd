...
```{r installation_code, echo = FALSE, eval = FALSE}
install.packages("devtools")
devtools::install_github("sjpierce/piercer")
devtools::install_github("sjpierce/SSACHR", ref = "main")
```
...
* Edit code pasted into `inst/Step_01_Data_mgt.Rmd` from previous 
  `CHR_Summary_Step01_2017-01.19.Rmd` script to:
    ...
    * Eliminate LaTeX Warning s such as "Label `tab:unnamed-chunk-8' multiply defined."
      that are caused by haivng multiple calls to kabl() in same code chunk. 
* Merge earliest SAK testing window date (WDate) onto each INC record, then 
  classify each ARR< CHG< JUD, and CON record as before, during, or after the
  the testing window. See 2020-04-08 email and subsequent emails.
* Summarize CHRs before, during, and after the testing window, per 2020-04-08 
  email thread.
* Create a folder structure diagram in `README.Rmd` and `README.md`. Check the 
  following web links for ideas how to do it. 
